...
validate_source() {
  ...
  require_contains "${ROOT_DIR}/systemd/showroom-agent.service" '^Environment=HOME=/var/lib/showroom$' "agent tools must not write through the locked root home"
  require_contains "${ROOT_DIR}/systemd/showroom-agent.service" '^Environment=XDG_CACHE_HOME=/var/lib/showroom/cache/\.tool-cache$' "agent tool cache must use persistent writable storage"
  require_contains "${ROOT_DIR}/config/config.env" '^SHOWROOM_YTDLP_MANAGED_VERSION=[0-9]{4}\.[0-9]{2}\.[0-9]{2}$' "managed yt-dlp release must be pinned"
